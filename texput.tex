...
\item {\bf Determination of the prediction context} ${\cal Q}$:
  ...
  \item Compute the local gradient:
    \begin{center}
      % \resizebox{3cm}{!}{\input{../figs/LOCO-I_contexto_prediccion}}
      % \texfigure{3cm}{!}{../figs/LOCO-I_contexto_prediccion}
      \includegraphics[width=3cm]{LOCO-I_contexto_prediccion}
    \end{center}
  ...
  \item Quantize the gradients:
    \begin{displaymath}
      q_i\leftarrow\left\{
        \begin{array}{ll}
          0 & \mathrm{if~} g_i=0\\
          1 & \mathrm{if~} 1\le |g_i|\le 2\\
          2 & \mathrm{if~} 3\le |g_i|\le 6\\
          3 & \mathrm{if~} 7\le |g_i|\le 14\\
          4 & \mathrm{otherwise}
        \end{array}
      \right.
    \end{displaymath}
...
\item {\bf Compute the residue} $M(e)$:
  ...
  \item The initial prediction:
    \begin{displaymath}
      \hat{s}\leftarrow\left\{
        \begin{array}{ll}
          \mathrm{min}(a,b) & \mathrm{if~} c\geq\mathrm{max}(a,b) \\
          \mathrm{max}(a,b) & \mathrm{if~} c\leq\mathrm{min}(a,b) \\
          a+b-c             & \mathrm{otherwise}.
        \end{array}
      \right.
    \end{displaymath}
  ...
  \item {\sl Bias} cancellation:
    \begin{displaymath}
      \hat{s}\leftarrow\left\{
        \begin{array}{ll}
          \hat{s}+C[Q] & \mathrm{if~} g_1>0 \\
          \hat{s}-C[Q] & \mathrm{otherwise}.
        \end{array}
      \right.
    \end{displaymath}
  ...
  \item Compute the prediction error:
    \begin{displaymath}
      e\leftarrow(s-\hat{s}) \mathrm{~mod~}\beta,
    \end{displaymath}
...
\item Recover the original Laplace distribution for the residues:
  \begin{displaymath}
    e\leftarrow M^{-1}(M(e))=\left\{
      \begin{array}{ll}
        -M(e)/2-1 & \mathrm{si~} M(e) \mathrm{is~odd}\\
        M(e)/2 & \mathrm{otherwise}.
      \end{array}
    \right.
  \end{displaymath}
\item $s\leftarrow (e+\hat{s})\mathrm{~mod~}\beta$.
